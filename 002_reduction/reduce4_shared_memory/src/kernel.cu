...
/**
 * n: length of the sequence to sum
 */
__global__ void reduce_yh(float *input, float *output, unsigned int n) {
  ...
  unsigned int left, right, stride;

  __shared__ float shared[1024];

  if (thread_id < block_size) {
    shared[thread_id] = input[block_start + thread_id] +
                        input[block_start + thread_id + block_size];
  }
...
__global__ void reduce(float *input, float *output, unsigned int n) {
  ...
  // Each thread adds 2 values, so each block adds a total of block_size * 2
  // values.
  unsigned int block_start = block_id * chunk_size;

  // Perform the reduction using "interleaved indexing" (each thread adds a pair
  // of elements right next to each other). "stride" is the distance between the
  // elements that each thread adds. This distance doubles on each iteration.
  ...
  unsigned int threads = block_size;
  for (unsigned int stride = 1; stride < chunk_size;
       stride *= 2, threads /= 2) {
    // There's a distance of stride between each pair of left and right operand
    // indices, so there's a distance of stride * 2 between consecutive left
    // indices
    ...
    if (thread_id < threads // read: "If this thread should be
        ...
        &&
        right <
            n) // If we're the last block, we may be running more threads
    ...
    // sync. The call below syncs the warps in the block at the end of each
    // iteration so that the results are written to memory before the next
    // iteration begins.
    __syncthreads();
  }

  // Once the loop is done, the partial sum for this block will be in the
  // leftmost index of this block's chunk. The code below causes each block's
  // thread 0 to write that partial result to the output buffer at position
  // "block_id". After the code below completes, the output buffer will contain
  // exactly <number of blocks> consecutive partial results.
  if (!thread_id) {
